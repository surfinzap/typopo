...
describe('Add non-breaking spaces after single-letter prepositions\n', () => {
	let testAllLang = {
		...
		"The product X is missing the feature Y.", // no nbsp after a single capital letter in the middle of the sentence
	};
...
describe('Fix non-breaking space around name with regnal number (sk, cs, de-de, rue)\n', () => {
	...
	Object.keys(unitTestCase).forEach((key) => {
		it("unit test", () => {
			...
			assert.strictEqual(fixNbspForNameWithRegnalNumber(key, new Locale("de-de")), unitTestCase[key]);
...
describe("Add nbsp before single capital letter in a sentence\n", () => {
	...
	Object.keys(testEnUs).forEach((key) => {
		it("unit tests (en-us)", () => {
			assert.strictEqual(
				addNbspBeforeSingleLetter(key, new Locale("en-us")),
				testEnUs[key]
			);
		});
	});
	Object.keys(testDeDeSkCs).forEach((key) => {
		it("unit tests (de-de)", () => {
			assert.strictEqual(
				addNbspBeforeSingleLetter(key, new Locale("de-de")),
				testDeDeSkCs[key]
			);
		});
	});
	Object.keys(testDeDeSkCs).forEach((key) => {
		it("unit tests (sk)", () => {
			assert.strictEqual(
				addNbspBeforeSingleLetter(key, new Locale("sk")),
				testDeDeSkCs[key]
			);
		});
	});
	Object.keys(testDeDeSkCs).forEach((key) => {
		it("unit tests (cs)", () => {
			assert.strictEqual(
				addNbspBeforeSingleLetter(key, new Locale("cs")),
				testDeDeSkCs[key]
			);
		});
	});
	Object.keys(testRue).forEach((key) => {
		it("unit tests (rue)", () => {
			assert.strictEqual(
				addNbspBeforeSingleLetter(key, new Locale("rue")),
				testRue[key]
			);
		});
	});
