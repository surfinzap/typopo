...
describe('Fix Initials (en-us)\n', () => {
	...
	Object.keys(testCase).forEach((key) => {
		...
		it('module test', () => {
			assert.strictEqual(
				fixAbbreviations(key, new Locale('en-us')),
				testCase[key]
			)
		})
	})
...
describe('Fix multiple-word abbreviations (sk, cs, rue, de-de)\n', () => {
	let testCase = {
		...
		'im Jahr 200 v. u. Z. als der Hunger':
			'im Jahr 200 v. u. Z. als der Hunger',
		...
		'Das Tier, d. h. der Fisch, lebte noch lange.':
			'Das Tier, d. h. der Fisch, lebte noch lange.',
		'Das Tier – d. i. der Fisch – lebte noch lange.':
			'Das Tier – d. i. der Fisch – lebte noch lange.',
		'Das Tier (d. h. der Fisch) lebte noch lange.':
			'Das Tier (d. h. der Fisch) lebte noch lange.',
	...
	Object.keys(testCase).forEach((key) => {
		it('unit test', () => {
			assert.strictEqual(
				fixMultipleWordAbbreviations(key, new Locale('sk')),
				testCase[key]
			)
		})
		it('module test', () => {
...
describe('Fix multiple-word abbreviations (en-us)\n', () => {
	...
	Object.keys(testCase).forEach((key) => {
		it('unit test', () => {
			assert.strictEqual(
				fixMultipleWordAbbreviations(key, new Locale('en-us')),
				testCase[key]
			)
		})
		it('module test', () => {
...
describe('Fix Single-word abbreviations (sk, cs, rue, de-de, en-us)\n', () => {
	...
	Object.keys(testCase).forEach((key) => {
		it('unit test', () => {
			assert.strictEqual(
				fixSingleWordAbbreviations(key, new Locale('sk')),
				testCase[key]
			)
		})
	})
